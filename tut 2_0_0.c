...
int main(int argc, char *argv[]){
    ...
    if(argc == 4){
        ...
        printf("Usage: %s N1 N2 M\n\n"
               " N1: matrix A0 N length\n"
               " N2: matrix A1 N length\n"
               " M:  matrix M length\n\n",argv[0]);
        return 1;
    }
...
/* Pairwise computation of two matrices. Each N i in A (rows1 by cols) is paired with every
   N j in B (rows2 by cols). The inner product of each pair (i, j) is computed and stored in
